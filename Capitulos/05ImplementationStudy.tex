...
\begin{resumen}
In this chapter, tasks related to the physical implementation of the platform are described. These tasks cover from the layout design, 3D modelling, and GERBER files generation, to the final components mounting on the PCB. Decisions about components or layout designs, and final results are exposed.  
\end{resumen}
...
\qquad
\subfloat[$\mu$Trans 434/868 PCB shape. 20.8x30 mm.]{\label{fig:cap5:mtransshape}\includegraphics[height=107px]{Imagenes/Vectorial/Capitulo5/mtransshape}}%
...
The board includes four holes for the battery to be attached. Figure \ref{fig:cap5:headersystem} ilustrates the way it works, for legs fix together the board and the batteries-holder. Moreover, the \ac{PCB} includes more holes for general purpose in case of feet or fixing are required.
...
Available \ac{UART}s at the headers are  **** and ****, presenting the first one hardware flow control and not presenting it the second one. \ac{UART} module chosen for serial communication is ************, because the communication does not require hardware flow control. The \ac{RX} signal is shorted to pin 21 and \ac{TX} signal to pin 38.
...
\subsection{Description}
%-------------------------------------------------------------------
This module is also a shield for the \ac{CNGD} module. The \ac{CNGD} sets the interface with the shield over the known expansion headers. Hence, this module must include the footprint defined in Figure \ref{fig:cap5:headerfootprint}. Both shields, rs232SHIELD and chargerSHIELD, use the shape ilustrated in Figure \ref{fig:cap5:rs232shape}.

Battery positive pole is accessible at the header on pin 29, negative pole is at pin 30. These two pins are linked to the circuit. Appart from this interface, the charger incrusts a terminal block to connect a external 12V source. This source must be capable to provide up to 500 mA.

Current version of the charger does not include any heat disipator. Tests showed that not heat disipator was required for the current configuration. Any change over the resistors configuration must consider the overheating possibility on LM317AMDT component. Thus a heat disipator could be needed.
...
First part of the chargerSHIELD to try is the LM311 amplificator, led, and surrounding components R1, R2, and R3. These parts constitute the comparator and with this, the comparator operation intends to be tested. Once this is checked, rest of components are soldered.

At this point, a lot of tests were carried out using different voltages and number of batteries to calibrate the system. The goal was to adjust the current over the batteries to a proper level, measure charging times and the proper operation of the notification led. R1, R2, R3, and R5 were finally set to their current value. Having this task achieved, external controlled source was swaped by a commercial 12V adaptor and its proper operation was proved. 
