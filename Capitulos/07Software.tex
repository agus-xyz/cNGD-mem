...
Adaptated example used polling to deal with the connection management, instead of interruptions. A circular buffer whould have been useful facing large volume of outputting data. So at the current state, the \ac{USB} tracing supposes an ideal choice for controlled environments where tracing tasks are taken carefully and localized. Otherwise data losses or even interferences with the communication protocol management migh arise. 

To configure the firmware for \ac{USB} traces, firstly it is needed to set the \\\emph{ENABLE\_CONSOLE} option. Then, the console must be stablished as \emph{DEBUG\_USB}. ****************** REQUIERE TECLA PARA EMPEZAR???
...
\item Bitrate for 434 MHz \ac{RI} is ********* bps, and 119.2 kbps for 868 MHz \ac{RI}.
...
\item At this point, antennas at 434 MHz \ac{RI}s are removed. 
...
\item At this point, antennas at 868 MHz \ac{RI}s are removed.
