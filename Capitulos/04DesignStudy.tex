...
Library is available at the mentioned repository hosting the hardware design files. All the content included into the library is ready to be used under Altium Designed. Any use of the library is free. 
...
Figure \ref{fig:cap4:rs232shieldschematic} shows the schematic for this shield. \ac{UART} 6 is used, accessible  at the headers and not requiring hardware flow control. Since RS-232 standard operates on voltage values from -12V to 12V, a level conversion is needed. This conversion takes place at the MAX3232\cite{max3232} transceiver. Figure \ref{fig:cap4:max3232} shows the MAX3232 scheme. \ac{TX} pin of the \ac{UART} is accessed at header pin 37, \ac{RX} is at 20. Since the communication does not require for hardware flow control\footnote{Hardware flow control allows to control the serial data flow by using extra signals Apart from the data signals.}, two pins are enough to set proper communication.
