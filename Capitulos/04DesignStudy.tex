...
\figura{Bitmap/Capitulo4/mrf24j40mablocks}{width=0.7\textwidth}{fig:cap4:mrf24j40mablocks}%
...
This regulator is configured to an output of 1.25V by chosing an R2? of 0 $\Omega$. Then a fix resistor between its output and ground will fix the output current and therefore, the input current. This fix current will flow through the batteries during the charge. 

The previously *********** output controls the ************* of a ******************** ¿transistor?. It switchs the transistor operation modes, ¿active and cut?, when ****** output is set to high or low, respectively.

 
%umbral, salida del op cambia
 
% afectando al transistor que conduce o no conduce

% further on transistor


% led se activa avisa de carga lista

% diodo precaucion

% se ha realizado simulacion del circuito.
...
%simulacion sobre altium designer. 
