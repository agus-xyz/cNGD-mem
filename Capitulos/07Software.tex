...
When adapting the mentioned firmware\ref{juanpfc} to the developed hardware platform, it was needed to deal with minor changes resulting from its novelty and also because of required hardware adaptations.
...
Other adaptations taken when developing software aim at establish better ease of use. An easier configuration scheme, \ac{USB} tracing modes, \ac{RI} power control functions, and new modules definition are the developed functionalities.

Current and older versions of the firmware, along with small software related tests, can be found at a public GitHub repository:\\ \emph{https://github.com/agus-xyz/cognitiveNGD}. Figure \ref{fig:cap7:repoqr} provides a \ac{QR} code to instantly access the repository.
...
\begin{itemize}
\item \emph{cNGD\_PLATFORM}. Automatically sets a suitable configuration for the \ac{CNGD}. \emph{MIWI\_0434\_RI} is established over \emph{MRF49XA\_1} transceiver, \emph{MIWI\_0868\_RI} is built over \emph{MRF49XA\_2}, and \emph{MIWI\_2400\_RI} does it on \emph{MRF24J40} transceiver. Remaining configurations to adequately take this options are automatically changed. 
...
Basically, the old macros that used to be changed manually for each platform are now automatically configured when set on of the previous options. Main changes take place over \ac{RI} configurations and pin-outs.
...
Adapted example offered possibilities for either polling or interruption methods, interruption management was chosen. A circular buffer would have been useful facing large volume of outputting data. So at the current state, the \ac{USB} tracing supposes an ideal choice for controlled environments where tracing tasks are taken carefully and localized. Otherwise data losses or even interferences with the communication protocol management might arise. 

To configure the firmware for \ac{USB} traces, firstly it is needed to set the \\\emph{ENABLE\_CONSOLE} option. Then, the console must be established as \emph{DEBUG\_USB}. Configuration file for \ac{USB} stack is \emph{include/USB/usb\_config.h}
...
%-------------------------------------------------------------------
After including power control modules for each \ac{RI} at the \ac{CNGD}, it was needed some software adaptations to fully make them operate. For this, a pair of functions have been included at the \ac{HAL}, \emph{SwitchRIOn(radioInterface ri)} and \emph{SwitchRIOff(radioInterface ri)}.
...
To access pins at the headers, applications can make use of the \emph{HEADERS\_XX} label definition and respective \emph{HEADERS\_XX\_TRIS} to configure the sort of pin. \emph{XX} parameter must be replaced with the desired pin number. For instance, \emph{HEADERS\_06\_TRIS} must be set as \emph{OUTPUT\_PIN} in order to make it a digital output. To access signal at header pin number six, the pin definition would be \emph{HEADERS\_06}.
...
A video showing the demo application working can be accessed at \\ \emph{http://tiny.cc/r5p14w}. For better comfort, Figure \ref{fig:cap7:demovideo} provides access to the video.
